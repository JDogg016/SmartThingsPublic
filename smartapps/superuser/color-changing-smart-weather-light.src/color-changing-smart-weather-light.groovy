...
def checkForWeather() {
	...
	//  FREEZING BLUE	
    if (conditions.current_observation.temp_f <= 65 ) {
        	color = "Blue"
        log.debug "Weather temp below 65F, its freezing out so setting light to blue."
    }
   	
    //  HOT ORANGE
	if (conditions.current_observation.temp_f >= 85 )  {
        color = "Orange"
        log.debug "Weather temp above 85F, setting light to orange."
    }
